...
function wdm_enqueue_front_script() {
		wp_enqueue_style( 'wdm_lightbox_css', plugins_url( 'lightbox/jquery.fs.boxer.css', __FILE__ ), array(), "1.0" );
		wp_enqueue_script( 'wdm-lightbox-js', plugins_url( 'lightbox/jquery.fs.boxer.js', __FILE__ ), array( 'jquery' ), 
			"1.9.0", array("in_footer" => false));
		wp_enqueue_script( 'wdm-block-ui-js', plugins_url( 'js/wdm-jquery.blockUI.js', __FILE__ ), array( 'jquery' ), "2.66.0-2013.10.09", array("in_footer" => false));
		wp_enqueue_script( 'wdm-custom-js', plugins_url( 'js/wdm-custom-js.js', __FILE__ ), array( 'jquery' ), "1.0",
...
function wdm_auction_listing( $atts = array() ) {
	...
	if ( $wdm_layout_style == 'layout_style_one' ) {
		wp_enqueue_style( 'wdm_auction_front_end_styling', plugins_url( 'css/ua-front-end-one.css', __FILE__ ), array(), 
			"1.0" );
	} else {
		wp_enqueue_style( 'wdm_auction_front_end_styling', plugins_url( 'css/ua-front-end-two.css', __FILE__ ), array(), 
			"1.0" );
	}
	...
	} elseif ( $currency_symbol == '$' || $currency_symbol == 'kr' ) {
	...
	if ( is_user_logged_in() && isset( $_GET['ult_auc_id'] ) && ! empty( $_GET['ult_auc_id'] ) && isset( $_GET['mt'] ) && ! empty( $_GET['mt'] ) ) {
		...
		esc_html_e( 'Thank you for buying this product.', 'wdm-ultimate-auction' );
		echo '<br /><br />';
		...
		printf( esc_html( __( 'You can make the payment by %s', 'wdm-ultimate-auction' ) ), esc_html( $mthd ) );

		if ( ! empty( $det ) ) {
			echo '<br /><br /><strong>' . esc_html( __( 'Details' ) ) . ':</strong> <br/>' . esc_html( $det );
		}

		echo '<br /><br /><a href="' . esc_url( get_permalink() ) . esc_html( $set_char ) . 'ult_auc_id=' . esc_attr( $_GET['ult_auc_id'] ) . '">' . esc_html( __( 'Go Back', 'wdm-ultimate-auction' ) ) . '</a>';

		$buy_now_price = get_post_meta( $wdm_auction->ID, 'wdm_buy_it_now', true );

		ultimate_auction_email_template(
			$wdm_auction->post_title,
			$wdm_auction->ID,
			$wdm_auction->post_content,
			$buy_now_price,
			$buyer_email,
			$ret_url
		);
		...
		if ( $wdm_auction ) {
			...
			$qry = $GLOBALS['wpdb']->get_var($wpdb->prepare("SELECT COUNT(bid) FROM {$wpdb->prefix}wdm_bidders WHERE auction_id = %d", $auctionid));
			...
			$bef_auc = '';
			$bef_auc = apply_filters(
				'wdm_ua_before_single_auction',
				$bef_auc,
				$wdm_auction->ID
			);
			echo wp_kses_post( $bef_auc ); // Use if $bef_auc contains HTML
			...
			<div class="wdm-ultimate-auction-container clearfix">
				...
				<div class="wdm-image-container clearfix">
					...
					for ( $c = 1; $c <= 4; $c++ ) {
						...
						if ( empty( $imgURL ) ) {
							...
							if ( ( ! is_null( $imgMime ) && strstr( $imgMime, 'image/' ) ) || in_array( $img_ext, $img_arr ) ) {
								...
								$images .= '<video class="auction-main-img" style="margin-bottom:0;" controls><source src="' . get_post_meta( $wdm_auction->ID, 'wdm-image-' . $c, true ) . '">Your browser does not support the video tag.</video>';
					...
					//echo wp_kses( $images, $allowed_html );
			...
			<div class="wdm_single_prod_desc clearfix">
				...
				if ( ( current_time( 'timestamp' ) >= strtotime( get_post_meta( $wdm_auction->ID, 'wdm_listing_ends', true ) ) ) || in_array( 'expired', $active_terms ) ) {
					...
					<div class="wdm-auction-ending-time">
						...
						printf( esc_html( __( 'Ended at', 'wdm-ultimate-auction' ) ) . ': ' . esc_html( __( '%s ago', 'wdm-ultimate-auction' ) ), '<span class="wdm-single-auction-ending">' . wp_kses_post( $rem_tm ) . '</span>' ); 
						?>							
					</div>
					...
					<?php if ( ! empty( $to_bid ) ) { ?>
						...
						if ( $no_bid == 1 ) {
							?>
								<strong><?php esc_html_e( 'No Bid', 'wdm-ultimate-auction' ); ?></strong>
								<?php
						} else {
							?>
									<strong><?php echo esc_html( $current_bid ); ?></strong>
								<?php } ?>
					...
					<div class="wdm-auction-ending-time">
						...
						/* translators: %s is remaining time */
						printf( esc_html( __( 'Ending in: %s', 'wdm-ultimate-auction' ) ), '<span class="wdm-single-auction-ending">' . wp_kses_post( $rem_tm ) . '</span>' ); 
						...
						<div id="wdm_place_bid_section" class="clearfix">
							...
							if ( is_user_logged_in() ) {
								...
								if ( $curr_user->ID != $wdm_auction->post_author ) {
									...
									$bidval_placeholder = sprintf(esc_html__( 'in %s', 'wdm-ultimate-auction' ), esc_html( $currency_symbol ) . esc_html( $currency_code_display ));
									?>
									<form action="<?php echo esc_attr( __DIR__ ); ?>" class="wdmua-singleauc-bidform">
										<div class="clearfix wdmua-clear">
											<div class="wdm_bid_val clearfix wdm-align-left">
												<label for="wdm-bidder-bidval" class="wdm-align-left wdmua-singleauc-label wdmua-singleauc-label-alt"><?php esc_html_e( 'Bid Value', 'wdm-ultimate-auction' ); ?>: </label>
												<input type="text" id="wdm-bidder-bidval" placeholder="<?php echo esc_attr($bidval_placeholder); ?>" value="<?php echo esc_attr( $curr_bid_val ); ?>" class="wdm-align-left wdmua-singleauc-input wdmua-singleauc-input-alt"/>
												<span class="wdm_enter_val_text wdm-align-right">
													<small>(<?php 

														/* translators: %.2f is minimum value to enter */
											...
											<div class="wdm_place_bid wdm-align-right clearfix">
												<input type="submit" 
													value="<?php echo esc_attr( apply_filters( 'wdm_ultimate_auction_bid_button_text', __( 'Place Bid', 'wdm-ultimate-auction' ) ) ); ?>" id="wdm-place-bid-now" />
											</div>
										</div>
									</form>
									<?php
								...
								if ( $check == 'without_login' ) {
									...
									/*printf( esc_html__( 'in %s', 'wdm-ultimate-auction' ), esc_html( $currency_symbol ) . esc_html( $currency_code_display ) );*/


									/* translators: %s is currency symbol */
									$wdm_placeholder_str = sprintf( esc_html__( 'in %s', 'wdm-ultimate-auction' ), esc_html( $currency_symbol ) . esc_html( $currency_code_display ) );
									...
									<div class="wdm_bidder_name wdmua-singleauc-fieldwrap clearfix">
										<label for="wdm-bidder-name" class="wdmua-singleauc-label wdm-align-left"><?php esc_html_e( 'Name', 'wdm-ultimate-auction' ); ?>: </label>
									...
									<div class="wdm_bidder_email wdmua-singleauc-fieldwrap clearfix">
										<label for="wdm-bidder-email" class="wdmua-singleauc-label wdm-align-left"><?php esc_html_e( 'Email', 'wdm-ultimate-auction' ); ?>:  </label>
									...
									<div class="clearfix">
										<div class="wdm_bid_val wdmua-singleauc-fieldwrap clearfix wdm-align-left">
											<label for="wdm-bidder-bidval" class="wdmua-singleauc-label wdm-align-left wdmua-singleauc-label-alt"><?php esc_html_e( 'Bid Value', 'wdm-ultimate-auction' ); ?>: </label>
										...
										<div class="wdm_place_bid clearfix wdm-align-right">
											<input type="submit" value="<?php echo esc_attr( apply_filters( 'wdm_ultimate_auction_bid_button_text', esc_html__( 'Place Bid', 'wdm-ultimate-auction' ) ) ); ?>" id="wdm-place-bid-now" class="wdm-align-right" />   
										</div>
									</div>
									...
									<div class="clearfix wdmua-clear">
										<div class="wdm_bid_val clearfix wdm-align-left">
											<label for="wdm-bidder-bidval" class="wdm-align-left wdmua-singleauc-label wdmua-singleauc-label-alt"><?php esc_html_e( 'Bid Value', 'wdm-ultimate-auction' ); ?>: </label>
											<input type="text" id="wdm-bidder-bidval" placeholder="<?php esc_attr($wdm_placeholder_str); ?>" class="wdm-align-left wdmua-singleauc-input wdmua-singleauc-input-alt"/>
											<span class="wdm_enter_val_text wdm-align-right">
												<small>(
												<?php

												/* translators: %.2f is minimum price to enter */
												printf( esc_html__( 'Enter %.2f or more', 'wdm-ultimate-auction' ),
													esc_html( $inc_price )
												);
												?>
												)</small>
										...
										<div class="wdm_place_bid clearfix wdm-align-right">
											<a class="wdm-login-to-place-bid wdm-align-right" href="#" title="<?php esc_attr_e( 'Login', 'wdm-ultimate-auction' ); ?>" data-login-url="<?php echo esc_url( $wdm_login_url ); ?>">
												<?php echo esc_html( apply_filters( 'wdm_ultimate_auction_bid_button_text', __( 'Place Bid', 'wdm-ultimate-auction' ) ) ); ?></a>
										</div>
									</div>
									<?php
								}
					...
					if ( ! empty( $to_buy ) || $to_buy > 0 ) {
						...
						if ( is_user_logged_in() ) {
								...
								<div id="wdm_buy_now_section" class="clearfix">
									<?php if ( $curr_user->ID != $wdm_auction->post_author ) { ?>
										<div id="wdm-buy-line-above" class="clearfix">
											<form action="<?php echo esc_html( $pp_link ); ?>" method="post" target="_top">
												<input type="hidden" name="cmd" value="_xclick">
												<input type="hidden" name="charset" value="utf-8" />
												<input type="hidden" name="business" value="<?php echo esc_attr( get_option( 'wdm_paypal_address' ) ); ?>">
												<!--<input type="hidden" name="lc" value="US">-->
												<input type="hidden" name="item_name" value="<?php echo esc_html( $wdm_auction->post_title ); ?>">
												...
												$wdm_buynowbtn_str = sprintf( esc_html__( 'Buy it now for %1$s%2$s %3$s', 'wdm-ultimate-auction' ), esc_html( $currency_symbol ), number_format( $buy_now_price, 2, '.', ',' ), esc_html( $currency_code_display ) );

												?>
												<input type="hidden" name="currency_code" value="<?php echo esc_attr( $currency_code ); ?>">
												<input type="hidden" name="return" value="<?php echo esc_url( get_permalink() ) . esc_html( $set_char ) . 'ult_auc_id=' . intval( $wdm_auction->ID ); ?>">
												<input type="hidden" name="button_subtype" value="services">
												<input type="hidden" name="no_note" value="0">
												<input type="hidden" name="bn" value="PP-BuyNowBF:btn_buynowCC_LG.gif:NonHostedGuest">
												<input type="submit" value="<?php echo esc_attr($wdm_buynowbtn_str); ?>" 
								...
								<script type="text/javascript">
									jQuery(document).ready(function () {
										jQuery("#wdm_buy_now_section form").click(function () {
											...
											jQuery("#wdm_buy_now_section form input[name='return']").val(cur_val + "&wdm=" + "<?php echo esc_html( $a_key ); ?>");
										});
										...
										<div id="wdm-buy-line-above" class="clearfix">
											...
											echo esc_url(
												add_query_arg(
													...
													get_permalink() . $set_char . 'ult_auc_id=' . $wdm_auction->ID
												)
											);
											?>
															" method="post">
												<input type="submit" value="<?php echo esc_attr( $bn_text ); ?>" id="wdm-buy-now-button">
											</form>
										</div>
									<?php } ?>
								</div>
								...
								<script type="text/javascript">
									jQuery(document).ready(function ($) {
										$("#wdm-buy-now-button").click(function () {											
											var bcnf = confirm('<?php echo esc_attr($wdm_buyitnow_btn_str); ?>');
							...
							<div id="wdm_buy_now_section" class="clearfix">
								<div id="wdm-buy-line-above" class="clearfix">
									<a class="wdm-login-to-buy-now" href="<?php echo esc_url( $wdm_login_url ); ?>" title="<?php esc_html_e( 'Login', 'wdm-ultimate-auction' ); ?>">
										<?php

										/* translators: %1$s is currency symbol, %2$s is buynow price, %3$s is currency code */
										printf( esc_html__( 'Buy it now for %1$s%2$s %3$s', 'wdm-ultimate-auction' ),
					...
					if ( is_user_logged_in() && $curr_user->ID == $wdm_auction->post_author ) {
						echo "<span class='wdm-align-left wdmua-clear wdmua-loggedin-error'>" . esc_html( __( 'Sorry, you can not bid on your own item.', 'wdm-ultimate-auction' ) ) . '</span>';
					}
			...
			<!--<div id="wdm_auction_desc_section">
					<div class="wdm-single-auction-description">
							<strong><?php // esc_html_e("Description", "wdm-ultimate-auction"); ?></strong>
							<br />
			...
			<script type="text/javascript">
				jQuery(document).ready(function ($) {
					...
					timer = setInterval(function () {
						...
						var ac_time = '<?php echo esc_html( $auc_time ); ?>';
						...
						var wdm_layout_style = "<?php echo esc_html( get_option( 'wdm_layout_style', 'layout_style_two' ) ); ?>";
						...
						if (wdm_layout_style == 'layout_style_one') {
								...
								if (vDays == 0) {
									...
									eDays.show();
									jQuery('#wdm_days_text').html(' <?php esc_html_e( 'day', 'wdm-ultimate-auction' ); ?> ');
								}
								else {
									eDays.show();
									jQuery('#wdm_days_text').html(' <?php esc_html_e( 'days', 'wdm-ultimate-auction' ); ?> ');
								}
								...
								if (vHours == 0) {
									...
									eHours.show();
									jQuery('#wdm_hrs_text').html(' <?php esc_html_e( 'hour', 'wdm-ultimate-auction' ); ?> ');
								}
								else {
									eHours.show();
									jQuery('#wdm_hrs_text').html(' <?php esc_html_e( 'hours', 'wdm-ultimate-auction' ); ?> ');
								}
								...
								if (vMinutes == 0) {
									...
									eMinutes.show();
									jQuery('#wdm_mins_text').html(' <?php esc_html_e( 'minute', 'wdm-ultimate-auction' ); ?> ');
								}
								else {
									eMinutes.show();
									jQuery('#wdm_mins_text').html(' <?php esc_html_e( 'minutes', 'wdm-ultimate-auction' ); ?> ');
								}
								...
								if (vSeconds == 0) {
									...
									eSeconds.show();
									jQuery('#wdm_secs_text').html(' <?php esc_html_e( 'second', 'wdm-ultimate-auction' ); ?>');
								}
								else {
									eSeconds.show();
									jQuery('#wdm_secs_text').html(' <?php esc_html_e( 'seconds', 'wdm-ultimate-auction' ); ?>');
								}
