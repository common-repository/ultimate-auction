	...
	class wdm_settings {
		...
		public function wdm_reg_post_contents() {
			...
			wp_enqueue_script( 'wdm_jq_validate', plugins_url( '/js/wdm-jquery-validate.js', __FILE__ ), array( 'jquery' ), 
				"1.19.5", array("in_footer" => false) );

			$trans_arr = array(
				'req' => __( 'This field is required.' ),
		...
		public function wdm_ajax_callback() {
			...
			if ( $_POST['show_rows'] == -1 ) {
				...
				$cache_key   = 'auction_results_' . $auctionid;
				$cache_group = 'auction_data';

				$results = wp_cache_get( $cache_key, $cache_group );
				if ( false === $results ) {
					$query = $GLOBALS['wpdb']->get_results($wpdb->prepare(
						"SELECT * FROM {$wpdb->prefix}wdm_bidders WHERE auction_id = %d 
					ORDER BY date DESC",
						$auctionid
					));
				} else {
		...
		public function add_plugin_page() {
			global $wp_version;
			if ( $wp_version >= '3.8' ) {
				$ua_icon_url = esc_url( plugins_url( 'img/favicon.png', __FILE__ ) );
			} else {
		...
		public function wdm_enqueue_style() {
			...
			if ( isset( $_GET['page'] ) && $_GET['page'] == 'add-new-auction' ) {
						...
						wp_enqueue_script( 'jquery-timepicker-js', plugins_url( '/js/date-picker.js', __FILE__ ), 
							array( 'jquery', 'jquery-ui-datepicker' ), "1.4.3", array("in_footer" => false) );
	...
	<!--code for displaying tabbed navigation-->
		...
		<h2 class="nav-tab-wrapper">
			<a href="?page=ultimate-auction" class="nav-tab <?php echo $active_tab == 'ultimate-auction' ? 'nav-tab-active' : ''; ?>">
				<?php esc_html_e( 'Settings', 'wdm-ultimate-auction' ); ?></a>
			<a href="?page=payment" class="nav-tab <?php echo $active_tab == 'payment' ? 'nav-tab-active' : ''; ?>">
				<?php esc_html_e( 'Payment', 'wdm-ultimate-auction' ); ?></a>
			...
			<a href="?page=add-new-auction" class="nav-tab <?php echo $active_tab == 'add-new-auction' ? 'nav-tab-active' : ''; ?>">
				<?php esc_html_e( 'Add Auction', 'wdm-ultimate-auction' ); ?></a>
			<a href="?page=manage_auctions" class="nav-tab <?php echo $active_tab == 'manage_auctions' ? 'nav-tab-active' : ''; ?>">
				<?php esc_html_e( 'Manage Auctions', 'wdm-ultimate-auction' ); ?></a>
			<?php do_action( 'ua_add_auction_tab', 'page', 'nav-tab', 'nav-tab-active', $active_tab ); ?>


			<a rel="nofollow" href="https://auctionplugin.net"  style="background-color: #04be5b;color: #fff" target="_blank" class="nav-tab <?php echo $active_tab == 'help-support' ? 'nav-tab-active' : ''; ?>"></i>
				<?php esc_html_e( 'Upgrade to PRO for more features', 'wdm-ultimate-auction' ); ?></a>
		</h2>
		<!--#code for displaying tabbed navigation-->
		...
		<form id="auction-settings-form" class="auction_settings_section_style" method="post" action="options.php">
				...
				$nonce_field = wp_nonce_field( 'ua_setting_wp_n_f', 'ua_wdm_setting_auc' );
	...
	<div class="uwa_setting_right">
					...
					<div class="text_uwa_setting">
						...
						<div class="happy_img">
							<a target="_blank" href="https://wordpress.org/support/plugin/ultimate-auction/reviews?rate=5#new-post">
								<img src="<?php echo esc_url( plugins_url( '/img/we_just_need_love.png', __FILE__ ) ); ?>" alt="<?php echo esc_attr( 'We Just Need Love' ); ?>" />
							</a>
						</div>
					</div>
				</div>
		...
		<div class="box_get_premium">
			<div class="get_premium">
				<div class="box_get_premium">
					<a rel="nofollow" href="https://auctionplugin.net?utm_source=ultimate plugin&utm_medium=vertical banner&utm_campaign=learn more" target="_blank">
					<img src="<?php echo esc_url( plugins_url( '/img/UWCA_col.jpg', __FILE__ ) ); ?>" alt="" />
						</a>
				</div>
			</div>
		</div>
	...
	<?php
		...
		public function wdm_page_init() {
			...
			if ( isset( $_GET['page'] ) && ( $_GET['page'] == 'ultimate-auction' ) && isset( $_GET['setting_section'] ) && ( $_GET['setting_section'] == 'auction' ) ) {
				...
				add_settings_field(
					'wdm_layout_style_id',
					__( 'Choose bid layout', 'wdm-ultimate-auction' ),
			...
			$trans_a = array(
				'pmt'  => __( 'You should fill data for atleast one payment method.', 'wdm-ultimate-auction' ),
		...
		public function wdm_validate_save_data( $input ) {
			...
			if ( isset( $_POST['ua_wdm_setting_auc'] ) && wp_verify_nonce(
				$_POST['ua_wdm_setting_auc'],
				'ua_setting_wp_n_f'
			) ) {
				...
				die( esc_html( __( 'Sorry, your nonce did not verify.', 'wdm-ultimate-auction' ) ) );
			}
			return $mid;
		}
		...
		public function wdm_auctions_num_per_page() {
			...
			?>
<input class="wdm_settings_input number required" min="1" type="number" size="5" id="wdm_auctions_page_num_id" name="wdm_auc_settings_data[wdm_auc_num_per_page]" value="<?php echo esc_html( get_option( 'wdm_auc_num_per_page' ) ); ?>" />
<div class="ult-auc-settings-tip">
			<?php esc_html_e( 'Please enter a number greater than or equal to 1.', 'wdm-ultimate-auction' ); ?>
</div>
			<?php
		}
		...
		public function wdm_email_field() {
			?>
			<input class="wdm_settings_input email required" type="text" id="wdm_email_id" name="wdm_auc_settings_data[wdm_auction_email]" value="<?php echo esc_html( get_option( 'wdm_auction_email' ) ); ?>" />
			<?php
		}
		...
		public function wdm_timezone_field() {
			...
			echo "<option value=''>" . esc_html( __( 'Select your Timezone', 'wdm-ultimate-auction' ) ) . '</option>';
			foreach ( $timezone_identifiers as $time_ids ) {
				$selected = ( get_option( 'wdm_time_zone' ) == $time_ids ) ? 'selected="selected"' : '';
				?>
				<option value="<?php echo esc_attr( $time_ids ); ?>" <?php echo esc_attr( $selected ); ?>>
					<?php echo esc_html( $time_ids ); ?>
				</option>
				<?php
			}
			echo '</select>';
			echo '<div class="ult-auc-settings-tip">' . esc_html( __( 'Please select your local Timezone.', 'wdm-ultimate-auction' ) ) . '</div>';
		}
	    ...
	    <option value="">' . esc_html( __( 'Simple Bidding', 'wdm-ultimate-auction' ) ) . '</option>';

			foreach ( $bidding_engine as $be ) {
				$select = get_option( 'wdm_bidding_engines' ) == $be['val'] ? 'selected' : '';
				echo '<option value="' . esc_attr( $be['val'] ) . '" ' . esc_attr( $select ) . '>' . esc_attr( $be['text'] ) . '</option>';
			}

			echo '</select>';
			echo '<div class="ult-auc-settings-tip">' . esc_html( __( 'Please select a default bidding engine.', 'wdm-ultimate-auction' ) ) . '</div>';
		}
		...
		public function wdm_currency_field() {
			...
			foreach ( $currencies as $currency ) {
				...
				if ( ! in_array( $currency, $pp_currencies ) ) {
					?>
					<option data-curr="<?php echo esc_attr( 'npl' ); ?>" value="<?php echo esc_attr( $currency ); ?>" <?php echo esc_attr( $selected ); ?>>
						<?php echo esc_html( $currency ); ?>
					</option>
					<?php
				} else {
					?>
					<option value="<?php echo esc_attr( $currency ); ?>" <?php echo esc_attr( $selected ); ?>>
						<?php echo esc_html( $currency ); ?>
					</option>
					<?php
				}
			...
			echo '</select>';
			echo ' <div id="nonpaypal" style="display: none; color: red;">' . esc_html( __( 'This currency is not available for PayPal.', 'wdm-ultimate-auction' ) ) . '</div>';
		}

		public function wdm_set_payment_options() {
			$default = array(
				'method_paypal'        => __( 'PayPal', 'wdm-ultimate-auction' ),
				'method_wire_transfer' => __( 'Wire Transfer', 'wdm-ultimate-auction' ),
				'method_mailing'       => __( 'Cheque', 'wdm-ultimate-auction' ),
				'method_cash'          => __( 'Cash', 'wdm-ultimate-auction' ),
			);
			...
			foreach ( $options as $key => $option ) {
				...
				<input
					...
					name="wdm_auc_settings_data[payment_options_enabled][<?php echo esc_attr( $key ); ?>]"
					type="checkbox"
					class="wdm_<?php echo esc_attr( $key ); ?>"/> 
			...
			esc_html_e( 'NOTE: If you choose to activate any payment method, please go to Payment tab and enter its details. For example: if you enable Wire Transfer, go to Payment -> Wire Transfer and enter its details. Same would apply to Paypal and Cheque.', 'wdm-ultimate-auction' );
		}

		// Auction feeder page URL
		public function wdm_auction_url_field() {
			?>
			<input type="text" class="wdm_settings_input url" id="wdm_auction_url_id" name="wdm_auc_settings_data[wdm_auction_page_url]" size="40" value="<?php echo esc_html( get_option( 'wdm_auction_page_url' ) ); ?>" />
			<div>
				<span class="ult-auc-settings-tip">
						<?php esc_html_e( 'Enter your auction feeder page URL.', 'wdm-ultimate-auction' ); ?></span>

				<a href="" class="auction_fields_tooltip"><strong>
						<?php esc_html_e( '?', 'wdm-ultimate-auction' ); ?></strong>
					<span style="width: 370px;margin-left: -90px;">
						<?php esc_html_e( "If you want to make each auction title as a link to the front end single auction page in 'Title' columns of the plugin dashboard, you'll need to enter front end URL of the page where you have used shortcode for auctions listing.", 'wdm-ultimate-auction' ); ?>
						<br /><br />
						<?php esc_html_e( 'NOTE: Whenever you change the permalink, do not forget to enter the modified URL here. Also, if you select auction page as Home page, do not enter Home page URL, instead use actual full URL of the feeder page.', 'wdm-ultimate-auction' ); ?>
					</span>
				</a>
				<br /><br />
				<span class="ult-auc-settings-tip">
						<?php esc_html_e( 'Use this shortcode in a page to make it auction feeder page:', 'wdm-ultimate-auction' ); ?></span>
						<?php echo esc_html( '<code>[wdm_auction_listing]</code>' ); ?>
			</div>
			<?php
		}
		...
		public function wdm_login_url_field() {
			?>
			<input type="text" class="wdm_settings_input url" id="wdm_login_url_id" name="wdm_auc_settings_data[wdm_login_page_url]" size="40" value="<?php echo esc_html( get_option( 'wdm_login_page_url' ) ); ?>" />
			<div>
				<span class="ult-auc-settings-tip">
						<?php esc_html_e( 'Enter Custom Login page URL (if have any).', 'wdm-ultimate-auction' ); ?></span>

				<a href="" class="auction_fields_tooltip"><strong>
						<?php esc_html_e( '?', 'wdm-ultimate-auction' ); ?></strong>
					<span style="width: 370px;margin-left: -90px;">
						<?php esc_html_e( 'If your site has a custom Login page and you want the bidders should log in through that page, you should set its URL here, so that while placing the bid, non logged in bidders should visit the custom Login page and not the default WordPress Login page. Please note, with the custom login URL the bidder will not be redirected automatically to the auction page where he/she was going to place the bid. As of now, this functionality works with the default WordPress Login page only. Also, whenever you change the permalink, do not forget to enter the modified URL over here.', 'wdm-ultimate-auction' ); ?>
						<br /><br />
						<?php esc_html_e( "NOTE: If your site uses default WordPress Login page. You don't need to set it.", 'wdm-ultimate-auction' ); ?>
					</span>
				</a>
			</div>
			<?php
		...
		public function wdm_register_url_field() {
			?>
		<input type="text" class="wdm_settings_input url" id="wdm_register_url_id" name="wdm_auc_settings_data[wdm_register_page_url]" size="40" value="<?php echo esc_html( get_option( 'wdm_register_page_url' ) ); ?>" />
		<div>
			<span class="ult-auc-settings-tip">
					<?php esc_html_e( 'Enter Custom Registration page URL (if have any).', 'wdm-ultimate-auction' ); ?></span>

			<a href="" class="auction_fields_tooltip"><strong>
					<?php esc_html_e( '?', 'wdm-ultimate-auction' ); ?></strong>
				<span style="width: 370px;margin-left: -90px;">
					<?php esc_html_e( 'If your site has a custom Register page and you want the bidders should register through that page, you should set its URL here, so that while placing the bid, non registered bidders should visit the custom Register page and not the default WordPress Register page. Also, whenever you change the permalink, do not forget to enter the modified URL over here.', 'wdm-ultimate-auction' ); ?>
					<br /><br />
					<?php esc_html_e( "NOTE: If your site uses default WordPress Register page. You don't need to set it.", 'wdm-ultimate-auction' ); ?>
				</span>
			</a>
		</div>
			<?php
		...
		public function wdm_show_enddate_field() {
			...
			foreach ( $options as $option ) {
				...
				echo '<input ' . esc_attr( $checked ) . " value='" . esc_attr( $option ) . "' name='wdm_auc_settings_data[wdm_show_enddate_msg]' type='radio' /> " . esc_attr( $option ) . " <br />";
			}
			printf( "<div class='ult-auc-settings-tip'>" . esc_html( __( 'Choose Yes if you want to display end date section.', 'wdm-ultimate-auction' ) ) . '</div>' );
		}
		...
		public function wdm_show_timezone_field() {
			...
			foreach ( $options as $option ) {
				...
				echo '<input ' . esc_attr( $checked ) . " value='" . esc_attr( $option ) . "' name='wdm_auc_settings_data[wdm_show_timezone_msg]' type='radio' />" . esc_attr( $option ) . " <br />";
			}
			printf( "<div class='ult-auc-settings-tip'>" . esc_html( __( 'Choose Yes if you want to display timezone section.', 'wdm-ultimate-auction' ) ) . '</div>' );
		}
		...
		public function wdm_show_prvt_msg_field() {
			...
			foreach ( $options as $option ) {
				...
				echo '<input ' . esc_attr( $checked ) . " value='" . esc_attr( $option ) . "' name='wdm_auc_settings_data[wdm_show_prvt_msg]' type='radio' /> " . esc_html( $option ) . " <br />";
			}
			printf( "<div class='ult-auc-settings-tip'>" . esc_html( __( 'Choose Yes if you want to display private message section.', 'wdm-ultimate-auction' ) ) . '</div>' );
		}
		public function wdm_allow_users_field() {
			...
			foreach ( $options as $option ) {
				...
				echo '<input ' . esc_attr( $checked ) . " value='" . esc_attr( $option ) . "' name='wdm_auc_settings_data[wdm_users_login]' type='radio' /> 
				" . esc_html( $opt ) . " <br />";
			}
			echo "<br /><div class='ult-auc-settings-tip'>" . sprintf( esc_html__( "%1\$sPLEASE NOTE:%2\$s 'Without login as well' option is not compatible with Shipping and Proxy bidding add ons for ultimate auction. Also, for 'Buy it now' option, login is still compulsory.", 'wdm-ultimate-auction' ), '<strong>', '</strong>' ) . '</div>';
		}
		...
		public function wdm_layout_style_field() {
			...
			foreach ( $options as $option ) {
				...
				echo '<input ' . esc_attr( $checked ) . " value='" . esc_attr( $option ) . "' name='wdm_auc_settings_data[wdm_layout_style]' type='radio' /> " . esc_html( $opt ) . " <br />";
			}
