...
// auction listing page using attributes - pagination
...
$show_content = '';
echo wp_kses_post( apply_filters( 'wdm_ua_before_auctions_listing', $show_content ) );
...
if ( $wdm_layout_style == 'layout_style_one' ) {
		...
		<ul class="wdm_auctions_list">
			...
			foreach ( $wdm_auction_array as $wdm_single_auction ) {
				...
				<li class="wdm-auction-single-item">
					<a href="
					<?php
					echo esc_url( get_permalink() ) . esc_html( $set_char ) . 'ult_auc_id=' .
					intval( $wdm_single_auction->ID );
					?>
					" class="wdm-auction-list-link">
						<ul class="clearfix">
								...
								<ul class="clearfix wdm-ua-af-list-inner-alt">
									...
									<li class="wdm-app auc_single_list auc_list_center wdm-ua-single-list wdm-ua-af-listing-alt">
										...
										<span class="wdm-auction-price wdm-mark-green">
											...
											if ( ( ! empty( $curr_price ) || $curr_price > 0 ) && ! empty( $ob ) ) {
												echo esc_html( $currency_symbol ) . number_format( $curr_price, 2, '.', ',' ) . ' ' . esc_html( $currency_code_display );
											} elseif ( ! empty( $ob ) ) {
												echo esc_html( $currency_symbol ) . number_format( $ob, 2, '.', ',' ) . ' ' .
									...
									<li class="wdm-ape auc_single_list auc_list_center wdm-ua-af-listing-alt wdm-ua-af-listing-alt-last">
										...
										if ( in_array( 'expired', $act_trm ) ) {
											...
											echo "<span class='wdm-mark-red'>" . esc_html__( 'Expired', 'wdm-ultimate-auction' ) . '</span>';
										}
										?>
										<br/>
									</li>
							...
							<li class="wdm-ua-desktop"><div class="wdm-apd"><?php echo wp_kses_post( $wdm_single_auction->post_excerpt ); ?> </div></li>
						</ul>
					</a>
				</li>
				<?php
	...
	include_once 'wdm-second_layout_list.php';
