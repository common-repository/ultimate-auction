...
function ultimate_auction_email_template(
	$auction_name,
	$auction_id,
	$auction_desc,
	$winner_bid,
	...
	$name_qry = $GLOBALS['wpdb']->get_var($wpdb->prepare(
		"SELECT name FROM {$wpdb->prefix}wdm_bidders WHERE bid = %d  AND 
		auction_id = %d AND email = %s ORDER BY id DESC",
		$winner_bid,
		$auction_id,
		$winner_email
	));
	...
	if ( $check_method === 'method_paypal' ) {
		...
		/* translators: %1$s is admin email, %2$ is payment amount */
...
function wdm_ua_outbid_notification_mail(
	$email,
	$bid,
	$ret_url,
	$auc_name,
	...
	$outbid_sub = '[' . get_bloginfo( 'name' ) . '] ' . __(
		'You have been outbid on the product',
		'wdm-ultimate-auction'
	) . ' - ' . $auc_name;
	$bid_msg    = '';
	$bid_msg    = __( 'Here are the details', 'wdm-ultimate-auction' ) . ' - ';
