document.addEventListener(
	...
	function () {
		...
		if (sliderJsVars.Totalimages > 0) {
			...
			prevButtonPopup.addEventListener(
				"click",
				() => {
			...
			nextButtonPopup.addEventListener(
				"click",
				() => {
			...
			*/
			...
			document.querySelectorAll('.thumbnails video').forEach(
				videoElement => {
			...
			// Handle click event on the current slide
			currentSlideContainer.addEventListener(
				"click",
				() => {
